...
export const fetchAppointmentRequests = async (selectedProviderType = 'Specialist') => {
    const queryParams = PassQueryParamsProvider.queryParams;
    try {
        let url = `/api/appointment-requests?provider_type=${selectedProviderType}&${new URLSearchParams(queryParams as any).toString()}`;
