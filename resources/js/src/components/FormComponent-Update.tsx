...
export default function FormComponentUpdate({ fields, initialData = {}, onSubmit, submitButtonText = '', formClassName = '', editViewClass = '', closemodal = () => {} }) {
    const [formData, setFormData] = useState(() => {
        return fields.reduce((acc, field) => {
            // Check first if initialData is not empty then assign initialData
            let fieldValue;
            switch (field?.type) {
                case 'select': {
                    if (!field.isLoading && field.options) {
                        field?.options?.map((option) => {
                            if (option?.value === initialData[field.name]) {
                                fieldValue = {
                                    value: option.value,
                                    label: option.label,
                                };
                ...
                default: {
                    fieldValue = initialData ? initialData[field.name] : field.initialValue || '';
                }
            }
            ...
            fields.reduce((acc, field) => {
                ...
                switch (field?.type) {
                    case 'select': {
                        ...
                        //     initialData[field.name] = typeof initialData[field.name][0] === 'object' ? initialData[field.name].map((item) => item?.id) : initialData[field.name];
                        //     field?.options?.map((option) => {
                        //         if (initialData[field.name].includes(option.value)) {
                        //             fieldValue = {
                        //                 value: option.value,
                        //                 label: option.label,
                        //             };
                        ...
                        //     return;
                        // }
                        // console.log(initialData[field.name]);
                        field?.options?.map((option) => {
                            if (option?.value === initialData[field.name]) {
                                fieldValue = {
                                    value: option.value,
                    ...
                    default: {
                        fieldValue = initialData ? initialData[field.name] : field.initialValue ? field.initialValue : '';
                    }
                }
                acc[field.name] = fieldValue || '';
                return acc;
